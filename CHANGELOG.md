...
- Item names in the loot browser carry a tooltip, the same as the popup's
...
- The "still needed" filter from the loot browser
...
- Each tracery your fellowship wins now shows on its own line in the popup, under the name the game printed, instead of everyone's being lumped into one "Tracery" line
- The loot browser lists "Tracery" as a single heading row rather than an item — everyone gets one and the game rolls its name, so there is nothing more specific to price
